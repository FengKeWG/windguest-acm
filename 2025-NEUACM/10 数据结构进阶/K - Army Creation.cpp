...
int query(int x, int y, int l, int r)
{
    if (l == r)
        return l;
    int lcnt = t[ls(y)].s - t[ls(x)].s;
    int rcnt = t[rs(y)].s - t[rs(x)].s;
    if (lcnt > k)
        return query(ls(x), ls(y), l, mid);
    if (rcnt > k)
...
signed main()
{
    ...
    {
        int l = read(), r = read();
        int id = r - l + 1 - query(root[l - 1], root[r], 1, bn);
        cout << r - l + 1 - query(root[l - 1], root[r], 1, bn) << endl;
    }
}